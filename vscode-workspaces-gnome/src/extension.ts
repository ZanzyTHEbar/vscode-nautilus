...
export default class VSCodeWorkspacesExtension extends Extension {
  ...
  enable() {
    ...
    this._newWindow =
      this.gsettings!.get_value("new-window").deepUnpack() ?? false;
    this._vscodeLocation =
      this.gsettings!.get_value("vscode-location").deepUnpack() ?? "code";
    this._refreshInterval =
      this.gsettings!.get_value("refresh-interval").deepUnpack() ?? 300;
  }
    ...
    try {
      ...
      while ((info = enumerator.next_file(null)) !== null) {
        try {
          ...
          const workspaceURI = (json.folder || json.workspace) as
            | string
            | undefined;
          if (!workspaceURI) {
          ...
          const workspaceExists = Array.from(this._workspaces).some(
            (workspace) => {
          ...
          if (workspaceExists) {
            log(
              `Workspace already exists in recent workspaces: ${workspaceURI}`,
            );
            continue;
          }
  ...
  _getRecentWorkspaces() {
    try {
      ...
      log(
        Array.from(this._recentWorkspaces).map((workspace) => workspace.path),
      );
    } catch (e) {
  ...
  _clearRecentWorkspaces() {
    ...
    try {
      ...
      const recentWorkspacesDir = Gio.File.new_for_path(
        this._recentWorkspacesPath,
      );
